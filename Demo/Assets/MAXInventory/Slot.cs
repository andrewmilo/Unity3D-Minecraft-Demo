			...
			if(inventoryElement != null)
			{
				if(inventoryElement.id > -1)
				{
					if(cachedItem == null)
					{
						...
						if(acceptedTypes.Exists(x => x.ID == inventoryElement.type.ID || x.isAncestorOf (inventoryElement.type)))
							inventoryElement.totalActions.FindAll (x => x.activateOnEquip).ForEach (x => inventoryElement.Use (x));
					}
				}
				...
				if(inventoryElement.name == "" && cachedItem != null)
				{
					if(acceptedTypes.Exists(x => x.ID == cachedItem.type.ID || x.isAncestorOf (cachedItem.type)))
					{
						...
						//For methods
						inventoryElement.totalActions.FindAll (x => x.activateOnUnEquip).ForEach (x => inventoryElement.Use (x));
					}
			...
			if(Application.isPlaying)
			{
				...
				if(activationCharacter != "None")
				{
					...
					if(Input.GetKeyDown (activationCharacter.ToLower ()))
					{
						...
						{
							foreach(ElementAction itemAction in inventoryElement.totalActions)
							{
						...
						if(ifActivateOnHotkey)
						{
							...
							if(Slot.activatedSlot != null)
							{
								...
								if(Slot.activatedSlot.inventoryElement != null)
									Slot.activatedSlot.inventoryElement.totalActions.ForEach (x => x.currentlyRepeating = false);

								foreach(ElementAction itemAction in Slot.activatedSlot.inventoryElement.totalActions)
								{
									//On Deactivation
							...
							{
								if(Slot.activatedSlot != null)
									Slot.activatedSlot.rectTransform.sizeDelta -= Slot.activatedSlot.changeSizeVector2;
				...
				if(Slot.activatedSlot == this)
				{
					...
					if(inventoryElement.name != "")
					{
						//For each action in each ancestor
						foreach(ElementAction itemAction in inventoryElement.totalActions)
						{
