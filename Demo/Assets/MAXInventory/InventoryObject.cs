...
public abstract class InventoryObject : MonoBehaviour
{
	...
	public bool isFull(InventoryElement element) 
	{
		for (int i = 0; i < Slots.Count; i++)
		{
			...
			else
			{
				...
				{
					if(slotItem.name == null || slotItem.name == "")
						return false;
	...
	public void AddItem(ref InventoryElement element, bool tryPreserveStack)
	{
		...
		for(int i = 0; i < Slots.Count; i++)
		{
			...
			foreach(ElementType x in slot.acceptedTypes)
			{
				if(x.isAncestorOf(element.type) || x.ID == element.type.ID)
				{
					match = true;
					break;
				}
			...
			if(slot.acceptedTypes.Count == 0 || match) 
			{
				//If there is no Item
				...
				else
				{
					...
					if((slotItem.prototype == element.prototype || slotItem.id == element.id) && InventoryManager.Instance.stackingActive)
					{
						if(slotItem.stack < slotItem.maxStack)
						{
							//Transfer
							while(slotItem.stack < slotItem.maxStack && element.stack > 0)
							{
								slotItem.stack++;
								element.stack--;
							}
